---
name: x:spec
description: "/x:spec, write spec, refine requirements, new feature, new page, CRUD, UI task. Use in this repo to run a spec-gated interview before x:plan or x:cook. Explores project context first, asks one focused question at a time with options, drafts a reviewed spec, and stops for user approval before handoff."
---

# Spec - Spec-Gated Interview

Use this repo-local `x:spec` skill to discover the right requirement before planning or implementation.

In this repo, treat this repo-local skill as the source of truth for `x:spec` behavior.
If older `prep` or `x:spec` variants under `.claude/` or `.agents/` conflict with this file, follow this file.
...
- before `x:plan` or `x:cook` for a new feature, page, flow, CRUD surface, or meaningful UI change
...
The goal is to define the request deeply enough that downstream `x:plan` can produce an end-to-end plan that stays close to the user's real intent.
...
Use a slug derived from the request so `x:plan` can auto-match it later.
...
Do not move to `x:plan`, `x:cook`, or implementation handoff until the user approves the spec.
...
After approval:
- save the final spec file to `docs/specs/YYYY-MM-DD-HHMM/<slug>-spec.md` unless the user explicitly asked for chat-only
...
/x:plan <one-line task summary> docs/specs/YYYY-MM-DD-HHMM/<slug>-spec.md
```
