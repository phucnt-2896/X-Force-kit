...
If the user only has an approved spec in chat and no saved spec file yet, prefer asking them to save or run `x:spec` first.
...
| :----------------------------- | :----------------------------------------------------------- | :---------------------------------------------------------------- |
| **Small plan, low-risk**       | `x:cook plans/{YYYYMMDD-HHMM}-{slug}/plan.md`              | Skip extra reviews; proceed directly to execution.                |
| **Complex plan, high-risk**    | Request user review/approval of the plan first, then `x:cook`. | High impact requires developer validation before generating code. |
...
/x:cook plans/{YYYYMMDD-HHMM}-{slug}/plan.md
```
...
To archive a plan (e.g., when requested as `/x:plan archive` or to archive completed work):
